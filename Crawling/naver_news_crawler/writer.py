    ...
    def initialize_file(self):
        ...
        if self.user_operating_system == "Windows":
            self.file = open("/Users/jungyulyang/programming/Project_ThematicInvest/Data/today_news_data/"
                             + self.category_name + '_' + str(self.save_start_year) + str(self.save_start_month)
                             + str(self.save_start_day) + '.csv', 'w', encoding='euc-kr', newline='')
        # Other OS uses utf-8
        else:
            self.file = open("/Users/jungyulyang/programming/Project_ThematicInvest/Data/today_news_data/"
                             + self.category_name + '_' + str(self.save_start_year) + str(self.save_start_month)
                             + str(self.save_start_day) + '.csv', 'w', encoding='utf-8',
                             newline='')
